...
class icoInfo(object):
    ...
    def getBlockData(self, ctx):
        ...
        for ico_name in ico_names.values()
            ...
            if icoStats: 
                ...
                total_output_value.update.update({ico_name: icoStats[0].total_output_value})
                meidan_transactions_value.update({ico_name: icoStats[0].meidan_transactions_value})
                block_time.update({ico_name: icoStats[0].block_time})
                block_count.updaet({ico_name: icoStats[0].block_count})
                block_pre_reward.update({ico_name: icoStats[0].block_pre_reward})
                total_transactions_fees.update({ico_name: icoStats[0].total_transactions_fees})
                avg_transactions_value.update({ico_name: icoStats[0].avg_transactions_value})
                difficulty.update({ico_name: icoStats[0].difficulty})
                hashrate.update({ico_name: icoStats[0].hashrate}) 
                blocks_mined_day.update({ico_name: icoStats[0].blocks_mined_day}) 
                bitcoins_mined_total.update({ico_name: icoStats[0].bitcoins_mined_total})
                median_confirmation_time.update({ico_name: icoStats[0].median_confirmation_time})
                mempool_size.update({ico_name: icoStats[0].mempool_size})
                current_best_transaction_fees.update({ico_name: icoStats[0].current_best_transaction_fees})
