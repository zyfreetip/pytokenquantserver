...
class EthDiSpider(Spider):
    ...
    def parse(self, response):
        ...
        blocks_avg_perhour = response.xpath('//tr[@id="t_blocksPerH"]/td[@class="coin c_eth"]/text()').extract()[0]
        item['blocks_avg_perhour'] = self.handle_string(blocks_avg_perhour)


        rewards_abbr  = response.xpath('//tr[@class="t_empty" and td[text()="Reward last 24h"]]/td[@class="coin c_eth"]/span/abbr')
        ...
        transaction_largest100 =  response.xpath('//tr[@class="t_empty" and td[text()="100 Largest Transactions"]]/td[@class="coin c_eth"]/span/text()').extract()[0]
        item['transaction_largest100'] = self.handle_string(transaction_largest100) * 100000000
        # address_numbers = response.xpath('').extract()[0]
        total = response.xpath('//tr[@id="t_total"]/td[@class="coin c_eth"]/text()').extract()[0]
