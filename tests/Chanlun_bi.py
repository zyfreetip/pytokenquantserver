...
def is_xianduan(last_direction, xianduan_list, fenbi):
    if fenbi[0].low < fenbi[1].high and fenbi[1].low < fenbi[3].high:
        direction = 1
        if not xianduan_list:
    ...
    elif fenbi[0].high > fenbi[1].low and fenbi[0].high < fenbi[3].low:
        direction = -1
        if not xianduan_list:
            xianduan_list.append(fenbi[0])
            xianduan_list.append(fenbi[3])
...
def run_test():
    ...
    df = pd.read_csv('BINANCE_BTCUSDT_20180131_1H.csv')
    ...
    has_result, result_array, point_index_list, fenbi_list =  fen_bi(merge_line_list)
run_test()
